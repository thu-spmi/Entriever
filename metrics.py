...
from KB_query import query_old # query 
...
def eval_end_to_end(test_data, turn_data=True, return_results=False):
    ...
    for dial in test_data:
        if turn_data:
            log = dial
            KB = dial[0]['KB']
            requested_info = dial[0]['goal'] # save info instead of goal, need to be modified
        else:
        ...
        for turn in log:
            ...
            gen_si=re.sub(r'(\(.*?\))', '', turn['客服意图-生成'])
            true_si=re.sub(r'(\(.*?\))', '', turn['客服意图'])
            gen_s, true_s=turn['客服-生成'].replace('[UNK]','g'), turn['客服']
