...
def train_ebm(cfg):
    ...
    for epoch in range(cfg.epoch_num):
        ...
        if not cfg.debugging:
            ...
            save_path = os.path.join(cfg.ebm_save_path, (model_name + ".pt"))
            print(save_path)
            if s > best_score:
...
def rescore(proposal_model, model, val_dataloader, hparams, tokenizer): # tokenizer
    ...
    accept_threshold = 1.0 # used to simplify computation, accept those triples above the Threshold and ignore them
